...
public extension UIColor {
    ...
    static var colorsSecondary: UIColor {
        let gradientLayer = CAGradientLayer()
        gradientLayer.colors = [UIColor(hex: "#FB6286").cgColor, UIColor(hex: "#9D5BFF").cgColor]
        gradientLayer.startPoint = CGPoint(x: 0.5, y: 0)
        gradientLayer.endPoint = CGPoint(x: 0.5, y: 1)
        
        let size = CGSize(width: 370, height: 56)
        UIGraphicsBeginImageContext(size)
        guard let context = UIGraphicsGetCurrentContext() else {
            return UIColor(hex: "#9D5BFF")
        }
    ...
    static var colorsPro: UIColor {
        let gradientLayer = CAGradientLayer()
        gradientLayer.colors = [UIColor(hex: "#9D5BFF").cgColor, UIColor(hex: "#FB6286").cgColor]
        gradientLayer.startPoint = CGPoint(x: 0.5, y: 0)
        gradientLayer.endPoint = CGPoint(x: 0.5, y: 1)
        
        let size = CGSize(width: 370, height: 30)
        UIGraphicsBeginImageContext(size)
        guard let context = UIGraphicsGetCurrentContext() else {
            return UIColor(hex: "#9D5BFF")
        }
