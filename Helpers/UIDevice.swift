...
extension UIDevice {
    ...
    static var isIphoneBelowX: Bool { // iPhone SE
        return UIScreen.main.bounds.height < 812 && UIDevice.current.userInterfaceIdiom == .phone
    }
    
    static var isIphoneXSeries: Bool { // iPhone X, Xs, 11 Pro
        let screenHeight = UIScreen.main.nativeBounds.height / UIScreen.main.nativeScale
        return (screenHeight == 812) && UIDevice.current.userInterfaceIdiom == .phone
    }
    
    static var isIphone15Pro: Bool { // iPhone 15 Pro
        let screenHeight = UIScreen.main.nativeBounds.height / UIScreen.main.nativeScale
        return screenHeight == 852 && UIDevice.current.userInterfaceIdiom == .phone
    }
